    ...
    def check_duel(self, duel: Duel) -> bool:
        ...
        if len(tables) > 3:
            ...
            if len(tables) > 3:
                ...
                logger.warning("More than 3 games found for %s", duel)
                return False

        if not duel.played_for_real:
            # Someone marked the game as not played
            return len(tables) == 0 and (
                (duel.p1_score == 2 and duel.p2_score == 0)
                or (duel.p1_score == 0 and duel.p2_score == 2)
            )
        ...
        if len(tables) < 2:
            ...
            if len(tables) < 2:
                ...
                logger.warning("Less than 2 games found for %s", duel)
                return False
        ...
        if duel.p1_score != p1_real_score or duel.p2_score != p2_real_score:
            logger.warning(
                "Wrong score for %s, got: %s - %s", duel, p1_real_score, p2_real_score
            )
            return False
