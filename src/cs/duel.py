...
from src.cs.date import date_timestamp, format_time
...
class Duel:
    ...
    def html(self):
        ...
        if self.p1_score is None:
            ...
            time_str = format_time(self.planned)
            return f"{p1_html} - {p2_html}: {time_str}"

        return (
            f'{self.p1.name} <a href="{self.url}">'
    ...
    def __repr__(self):
        ...
        odate = f"'{self.outcome_timestamp}'" if self.outcome_timestamp else "None"

        return (
            f"Duel({p1_str}, {p2_str}, {pdate}, {sdate}, {odate},"
            f"{self.p1_score}, {self.p2_score})"
        )
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            ...
            and self.schedule_timestamp == other.schedule_timestamp
            and self.outcome_timestamp == other.outcome_timestamp
        )
