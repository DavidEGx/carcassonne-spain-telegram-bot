...
class GCalendar(IoBase):
    ...
    def events(self) -> list[dict[str, Any]]:
        ...
        events_result = self.event_mgr.list(calendarId=self.calendar_id,
                                            timeMin=start,
                                            maxResults=1000,
                                            singleEvents=True,
    ...
    def create_msg(self,
                   query_date: date,
                   force_schedule: bool = False) -> list[dict[str, Any]]:
    ...
    def send(self,
        ...
        logger.info(f"Creating events for {query_date}")
        msgs = self.create_msg(query_date, force_schedule)

        for msg in msgs:
            event = self._find_event(msg['summary'])
            if event:
                logger.debug(f"Updating event {event['id']} {msg['summary']}")
                self.event_mgr.update(calendarId=self.calendar_id,
                                      eventId=event['id'],
                                      body=msg).execute()
            else:
                logger.debug(f"Inserting event {msg['summary']}")
                self.event_mgr.insert(calendarId=self.calendar_id,
                                      body=msg).execute()
